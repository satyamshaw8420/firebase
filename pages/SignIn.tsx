...
import { signInWithGooglePopup, signInWithGoogleRedirect, handleRedirectResult } from '../firebase/authService';
...
const SignIn: React.FC = () => {
  ...
  useEffect(() => {
    const checkRedirect = async () => {
      try {
        ...
        if (user) {
          console.log("User successfully logged in via redirect:", user);
  ...
  const handleButtonMouseMove = (e: React.MouseEvent) => {
    if (googleButtonRef.current) {
      ...
      const depth = (1 - distance / maxDistance) * 25;
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (user) {
        console.log("User successfully logged in via Popup:", user);
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        setError("Sign-in window was closed. If popups are blocked, try clicking again or use another browser.");
      } else if (err.code === 'auth/cancelled-popup-request') {
        // Silently handle
      } else if (err.code === 'auth/popup-blocked') {
        setError("Popup blocked. Redirecting to secure sign-in...");
