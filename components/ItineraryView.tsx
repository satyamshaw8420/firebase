...
export const ItineraryView: React.FC<ItineraryViewProps> = ({ 
  ...
  const downloadAsPDF = () => {
    try {
      ...
      // Add title
      pdf.setFontSize(22);
      pdf.setTextColor(1, 95, 99); // Deep teal color
      pdf.setFont('helvetica', 'bold');
      pdf.text(editedData.tripName, pageWidth / 2, 20, { align: 'center' });
      
      // Add subtitle
      pdf.setFontSize(14);
      pdf.setTextColor(100);
      pdf.setFont('helvetica', 'normal');
      pdf.text(editedData.destinationOverview, pageWidth / 2, 30, { align: 'center' });
      
      // Add cost information
      pdf.setFontSize(12);
      pdf.setTextColor(0);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`Total Estimated Cost: ${editedData.currency} ${editedData.totalEstimatedCost.toLocaleString()}`, 20, 45);
      pdf.text(`Per Person Cost: ${editedData.currency} ${perPersonCost.toLocaleString()}`, 20, 52);
      
      // Add a line separator
      pdf.setDrawColor(1, 95, 99);
      ...
      editedData.dailyItinerary.forEach((day, dayIndex) => {
        ...
        // Day header
        pdf.setFontSize(16);
        pdf.setTextColor(1, 95, 99);
        pdf.setFont('helvetica', 'bold');
        pdf.text(`Day ${day.day}: ${day.theme}`, 20, yPos);
        yPos += 8;
        ...
        day.activities.forEach((activity, actIndex) => {
          ...
          // Activity time
          pdf.setFont('helvetica', 'bold');
          pdf.text(activity.time, 25, yPos);
          
          // Activity name
          pdf.setFont('helvetica', 'bold');
          pdf.text(activity.activity, 45, yPos);
          yPos += 6;
          
          // Activity description
          pdf.setFont('helvetica', 'normal');
          const descriptionLines = pdf.splitTextToSize(activity.description, pageWidth - 50);
          for (let i = 0; i < descriptionLines.length; i++) {
            if (yPos > pageHeight - 30) {
              pdf.addPage();
              yPos = 20;
            }
            pdf.text(descriptionLines[i], 45, yPos);
            yPos += 5;
          }
          
          // Activity location
          pdf.setFont('helvetica', 'italic');
          pdf.text(`Location: ${activity.location}`, 45, yPos);
          yPos += 6;
          
          // Estimated cost
          pdf.setFont('helvetica', 'normal');
          pdf.text(`Estimated Cost: ${editedData.currency} ${activity.estimatedCost}`, 45, yPos);
          yPos += 10;
        });
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.text('Generated by TravelEase', 20, pageHeight - 10);
      }
      
      // Download the PDF
      pdf.save(`TravelEase_Itinerary_${editedData.tripName || 'Trip'}.pdf`);
  ...
  return (
      <div className="max-w-5xl mx-auto px-4 py-8 relative">
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-bottom-4">
            ...
            {/* Header Section */}
            <div id="itinerary-content" className="p-8 text-white relative overflow-hidden" style={{ backgroundColor: COLORS.primary }}>
